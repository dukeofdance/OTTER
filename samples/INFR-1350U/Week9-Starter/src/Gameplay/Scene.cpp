...
entt::handle GameScene::CreateEntity(const std::string& name) {
	entt::entity result = _registry.create();
	auto& transform = _registry.emplace<Transform>(result);
	auto& tag = _registry.emplace<GameObjectTag>(result, name);
	return entt::handle(_registry, result); 
}
